...
It is deliberately built to generate a **large plan (~40 MB of machine-readable
...
make size ENV=prod        # ~40 MB plan.json, ~15.5k resources
...
terraform show -json tfplan.bin > plan.json   # <- the ~40 MB artifact
terraform show tfplan.bin        > plan.txt   # human-readable (~30 MB)
```
...
| ------------------------------------- | --------- | -------------------------------------- |
| `terraform show -json tfplan.bin`     | **~40 MB** | Machine-readable plan (OPA/Sentinel)   |
| `terraform show tfplan.bin` (text)    | ~30 MB    | Human-readable diff                    |
...
and CI pipelines, so that is what is tuned to ~40 MB.
...
~15,500 resources for ~40 MB). Scale the service instance counts up or down
proportionally to hit a different target.
