...
It is deliberately built to generate a **large plan (~20 MB of machine-readable
...
make size ENV=prod        # ~20 MB plan.json, ~8k resources
...
terraform show -json tfplan.bin > plan.json   # <- the ~20 MB artifact
terraform show tfplan.bin        > plan.txt   # human-readable (~15 MB)
```
...
| ------------------------------------- | --------- | -------------------------------------- |
| `terraform show -json tfplan.bin`     | **~20 MB** | Machine-readable plan (OPA/Sentinel)   |
| `terraform show tfplan.bin` (text)    | ~15 MB    | Human-readable diff                    |
...
and CI pipelines, so that is what is tuned to ~20 MB.
...
Roughly: **~8,000 resources ≈ 20 MB** of JSON. Scale the service instance counts
up or down proportionally to hit a different target.
